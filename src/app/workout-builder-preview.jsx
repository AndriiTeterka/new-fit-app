...
// Preview-only version of the new-mobile Workout Builder UI
export default function WorkoutBuilderPreviewScreen() {
  ...
  // Mock save (no real API) so you can preview the UI without backend
  const createWorkoutMutation = useMutation({
    mutationFn: async (workoutData) => {
      // simulate latency
      await new Promise((r) => setTimeout(r, 600));
      return { id: "preview", ...workoutData };
    },
    onSuccess: () => {
      Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
      Alert.alert("Success", "Workout saved (preview)", [
  ...
  const addExercise = () => {
    // Preview-only: just add a mock exercise instead of navigating
    setExercises((prev) => [
      ...prev,
      {
        id: Date.now(),
